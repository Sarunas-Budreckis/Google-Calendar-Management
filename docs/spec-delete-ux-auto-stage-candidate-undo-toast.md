...
created: '2026-05-13'
status: 'done'
baseline_commit: '4f93efbb3ef7f3f822b4ca1ce5f1eb5f9a4a872d'
context: []
---
...
**Execution:**
- [x] `Messages/RequestUndoToastMessage.cs` -- CREATE new sealed record with `string Message` and `Func<CancellationToken, Task> OnUndo` -- carries the undo callback from `EventDetailsPanelViewModel` to `MainViewModel`
- [x] `ViewModels/EventDetailsPanelViewModel.cs` -- EDIT `DeleteEventAsync()`: (1) in the `CalendarEventSourceKind.Pending` branch, remove `ShowConfirmationAsync` call and its guard; capture a snapshot of `_pendingEventId` and the `PendingEvent` entity before deleting; after delete+hide+clear+send, send `RequestUndoToastMessage` whose `OnUndo` re-inserts the snapshot via `_pendingEventRepository.UpsertAsync` then sends `EventUpdatedMessage`; (2) in the GCal no-pending-edit branch, remove `ShowConfirmationAsync("Stage Deletion", ...)` and its guard, proceeding directly to upsert -- needed to remove friction
- [x] `ViewModels/MainViewModel.cs` -- EDIT: add `bool IsUndoToastVisible`, `string UndoToastMessage` observable properties; add private `Func<CancellationToken, Task>? _pendingUndoAction`; add `UndoCommand` async relay command that calls `_pendingUndoAction` then `DismissUndoToast()`; add `DismissUndoToast()` public method that clears `_pendingUndoAction` and sets `IsUndoToastVisible = false`; register `RequestUndoToastMessage` handler that stores the callback, sets message, and sets `IsUndoToastVisible = true` -- needed to drive toast visibility from the ViewModel
- [x] `Views/MainPage.xaml` -- EDIT: inside the `<Grid Grid.Row="2">`, added `Border` as last child in `Grid.Column="1"` with `UndoToastVisibility` binding, horizontal `StackPanel` with message `TextBlock` and Undo `Button` -- needed for bottom-center floating toast UI
- [x] `Views/MainPage.xaml.cs` -- EDIT: add `DispatcherQueueTimer? _undoToastTimer`; in `ViewModel_PropertyChanged` wire `IsUndoToastVisible` to `UpdateUndoToastTimer()`; add `UpdateUndoToastTimer()`, `CreateUndoToastTimer()`, `UndoToastTimer_Tick()` -- needed for auto-dismiss without timer logic in the ViewModel
...
**Delete flow changes (entry point)**

- Candidate delete: debounce stopped first, snapshot captured, then delete + undo toast sent
  [`EventDetailsPanelViewModel.cs:1703`](../ViewModels/EventDetailsPanelViewModel.cs#L1703)

- GCal staging: confirmation guard removed, proceeds directly to upsert
  [`EventDetailsPanelViewModel.cs:1728`](../ViewModels/EventDetailsPanelViewModel.cs#L1728)

**Cross-VM undo handoff**

- New message record carrying the undo callback to `MainViewModel`
  [`RequestUndoToastMessage.cs:1`](../Messages/RequestUndoToastMessage.cs#L1)

- Handler stores callback and raises `IsUndoToastVisible`; `ExecuteUndoAsync` clears before calling (race-safe)
  [`MainViewModel.cs:1064`](../ViewModels/MainViewModel.cs#L1064)

- `DismissUndoToast` public method clears callback and hides toast
  [`MainViewModel.cs:460`](../ViewModels/MainViewModel.cs#L460)

**Toast UI**

- Bottom-center overlay `Border` in `Grid.Column="1"`, last child (renders on top)
  [`MainPage.xaml:527`](../Views/MainPage.xaml#L527)

- 5-second auto-dismiss timer wired to `IsUndoToastVisible` property change
  [`MainPage.xaml.cs:727`](../Views/MainPage.xaml.cs#L727)
