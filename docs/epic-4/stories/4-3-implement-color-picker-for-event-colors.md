...
- Added a canonical 9-color picker contract with ordered picker metadata, canonical key normalization, and Azure fallback for null, unknown, and legacy numeric IDs.
...
- Replaced the Story 4.1 "Coming soon" color stub with an anchored `Flyout` picker that renders 9 swatches in the AC-required 2×6 layout, exposes accessible names, and visibly highlights the current selection.
