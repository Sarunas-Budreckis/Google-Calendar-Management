...
## Epic 4: Event Editing (Tier 2)

**Goal:** Enable users to create and edit calendar events locally before publishing to Google Calendar

**Note:** Stories 4.1, 4.2, and 4.3 were originally drafted as Epic 3 stories (3.5, 3.6, 3.7) and moved to Epic 4 during sprint restructuring on 2026-04-05. They are fully drafted and ready for development. Full story files are in `docs/epic-4-event-edit-push-to-gcal/stories/`.
...
**Acceptance Criteria:** See [docs/epic-4-event-edit-push-to-gcal/stories/4-1-implement-event-editing-panel.md](epic-4-event-edit-push-to-gcal/stories/4-1-implement-event-editing-panel.md)

**Prerequisites:** Stories 3.2, 3.3, 3.4

**Technical Notes:**
- Extends `EventDetailsPanelViewModel` and `EventDetailsPanelControl` from Story 3.4
- Auto-save with 500 ms `DispatcherTimer` debounce
- Writes directly to `gcal_event` table (no `pending_event` in this story)
- Single-level Undo (Ctrl+Z)
- Color picker stub only (replaced by Story 4.3)
...
As a **user**,
I want **to create new events directly on the calendar**,
so that **I can capture local-only events immediately**.

**Acceptance Criteria:** See [docs/epic-4-event-edit-push-to-gcal/stories/4-2-implement-event-creation-drag-to-create-and-button.md](epic-4-event-edit-push-to-gcal/stories/4-2-implement-event-creation-drag-to-create-and-button.md)

**Prerequisites:** Stories 4.1, 3.3, 3.4

**Technical Notes:**
- Introduces `pending_event` table for new local-only drafts (cannot use `gcal_event` — no Google ID yet)
- Drag-to-create in Week and Day views only (15-minute snap)
- `+ Add Event` toolbar button with `ContentDialog`
- Widens shared event identity contracts from Google-only to source-agnostic
- Renders pending drafts at 60% opacity with Azure default color
...
As a **user**,
I want **to assign an event color from a fixed palette while editing**,
so that **I can categorize events visually**.

**Acceptance Criteria:** See [docs/epic-4-event-edit-push-to-gcal/stories/4-3-implement-color-picker-for-event-colors.md](epic-4-event-edit-push-to-gcal/stories/4-3-implement-color-picker-for-event-colors.md)

**Prerequisites:** Story 4.1

**Technical Notes:**
- Replaces Story 4.1's disabled color stub with a real `Flyout` picker
- Fixed palette of exactly 9 colors (Azure, Purple, Grey, Yellow, Navy, Sage, Flamingo, Orange, Lavender)
- Immediate save on selection (bypasses 500 ms text debounce)
- Writes version history snapshot to `gcal_event_version` before overwrite (`ChangedBy = "manual_edit"`)

---

**Epic 4 Sequencing:** Depends on Epic 3 (specifically Stories 3.2–3.4). Begin after Tier 1 (Epic 3) is stable.

**What's NOT in Epic 4:**
❌ Push to Google Calendar (Epic 7)
❌ Event deletion
...
**Goal:** Introduce the data source management surface — a persistent left panel showing data source status and per-day coverage tracking — along with the single-day selection model that drives it. Deliver the first working data source (Toggl Track Sleep) end-to-end to validate the infrastructure.

**Full scope:** See [docs/epic-5-day-select-left-data-panel/epic-overview.md](epic-5-day-select-left-data-panel/epic-overview.md)
...
- 5.5: Left panel day mode (date/name header; coverage checkboxes; expand/drilldown navigation)
...
**Goal:** Enable the user to collect a non-contiguous set of days into a persistent batch and perform bulk operations (approve, mark coverage, select events), with Next/Previous navigation that drives the left panel — dramatically accelerating the backfilling ritual.

**Full scope:** See [docs/epic-6-batch-multi-select/epic-overview.md](epic-6-batch-multi-select/epic-overview.md)
...
- 6.6: Batch mark coverage action (source picker; bulk write to legacy `date_source_integration`)
...
> **[2026-06-11 Note]** Story 6.6 ("mark coverage") is superseded by computed **coverage** in Epic 8 (the `date_source_integration` table is being deleted). Re-scope or drop 6.6 when Epic 6 is contexted. See [epic-8-data-linking/concepts.md](epic-8-data-linking/concepts.md).
