...
Google Chrome search history is stored locally in a SQLite database at `%LOCALAPPDATA%\Google\Chrome\User Data\Default\History`. It can also be exported via Google Takeout (as part of Chrome data) as a JSON file. Candidate event (autogenerated) generation and query filtering are deferred to future stories. This story imports the full history (from one of these two sources) and stores it locally.
...
- This is investigatory in Epic 7 in the sense that candidate event (autogenerated) generation is not implemented; the data model and import are the deliverable
