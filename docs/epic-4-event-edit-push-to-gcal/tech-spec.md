...
public sealed record CalendarColorOption(
    string Key,            // canonical key: "red", "navy", "azure", …
    string DisplayName,    // "Red", "Navy", "Azure", …
    string Hex,            // "#D50000", "#3F51B5", "#3CABFF", …
    string ContrastTextHex // always "#FFFFFF" for all 12 colors
);
```
...
public interface IColorMappingService
{
    ...
    IReadOnlyList<CalendarColorOption> PickerColors { get; }  // 12 entries, ordered for picker
    string GetDisplayName(string? colorId);
    string NormalizeColorKey(string? rawId);                  // "1" → "lavender"; unknown → "azure"
}
```

### Canonical Color Table

| Canonical Key | Display Name | Google ID | Hex | Contrast Text |
|---|---|---|---|---|
...
Google Calendar numeric aliases resolved by `NormalizeColorKey`: `"1"` → `lavender`, `"2"` → `sage`, `"3"` → `grape`, `"4"` → `flamingo`, `"5"` → `banana`, `"6"` → `orange`, `"7"` → `peacock`, `"8"` → `graphite`, `"9"` → `navy`, `"10"` → `basil`, `"11"` → `red`.
...
└── EditView (VisualState: EditMode)
    ...
    │           └── Flyout: 2×6 CalendarColorOption grid (2 rows × 6 columns; 12 swatches)
...
→ User clicks a color option (e.g. "Navy")
...
- `ColorMappingService_PickerColors_ContainsExactlyTwelveEntriesInOrder`
