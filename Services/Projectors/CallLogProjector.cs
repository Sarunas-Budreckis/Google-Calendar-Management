...
public sealed class CallLogProjector : IDataPointProjector
{
        ...
        CancellationToken ct = default)
    {
        ...
        var entries = await ctx.CallLogEntries.AsNoTracking().ToListAsync(ct);
    ...
    public async Task<IReadOnlyList<string>> GetAllRawSourceRefsAsync(
        ...
        CancellationToken ct = default)
    {
        var ids = await ctx.CallLogEntries
            .AsNoTracking()
            .Select(e => e.Id)
            .ToListAsync(ct);
    ...
    private DataPointSpec Project(CallLogEntry entry)
    {
        var endUtc = entry.DurationSeconds > 0
            ? entry.Date.AddSeconds(entry.DurationSeconds)
            : entry.Date;

        return new(SourceKey, SourceRef(entry), entry.Date, endUtc);
    }

    private static string SourceRef(CallLogEntry entry) => entry.Id.ToString();
}
