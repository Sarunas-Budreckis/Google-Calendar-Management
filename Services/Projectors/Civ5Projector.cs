...
public sealed class Civ5Projector : IDataPointProjector
{
        ...
        CancellationToken ct = default)
    {
        ...
        var points = await ctx.Civ5SessionPoints.AsNoTracking().ToListAsync(ct);
    ...
    public async Task<IReadOnlyList<string>> GetAllRawSourceRefsAsync(
        ...
        CancellationToken ct = default)
    {
        var ids = await ctx.Civ5SessionPoints
            .AsNoTracking()
            .Select(p => p.Id)
            .ToListAsync(ct);
    ...
    private DataPointSpec Project(Civ5SessionPoint point) =>
        new(SourceKey, SourceRef(point), point.FileModifiedAt, point.FileModifiedAt);

    private static string SourceRef(Civ5SessionPoint point) => point.Id.ToString();
}
