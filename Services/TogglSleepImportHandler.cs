...
public sealed class TogglSleepImportHandler : IDataSourceImportHandler
{
    ...
    public async Task TriggerImportAsync(CancellationToken ct = default)
    {
        ...
        await _dialogService.ShowErrorAsync(
            "Toggl Sleep Import",
            result.ErrorMessage ?? "Unable to import Toggl sleep entries.");
    }

    private async Task<(DateOnly From, DateOnly To, TogglSleepImportResult? Result)?> ShowDateRangeDialogAsync(CancellationToken ct)
    {
        ...
        var content = new StackPanel
        {
            ...
            Children =
            {
                new TextBlock
                {
                    Text = "Choose the inclusive date range to import from Toggl Track.",
        ...
        TogglSleepImportResult? importResult = null;
